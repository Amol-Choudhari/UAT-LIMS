...
class DmiSmsEmailTemplatesTable extends Table{
		...
		if (!empty($find_message_record)) {
			...
			if ($getUserId == 101) {
				...
				if (!empty($inward_data)) {
					...
					if (trim($inward_data['role']) == 'Inward Officer') {
						...
						//This is added on 27-04-2021 for base64decoding by AKASH
			...
			if (!empty($find_message_record['sms_message'])) {
				...
				*/	
				...
				$DmiSentSmsLogsEntity = $DmiSentSmsLogs->newEntity(array(
					...
					'destination_list'=>base64_encode($log_dest_mob_nos_values),
					'mid'=>null,
			...
			if (!empty($find_message_record['email_message'])) {
				...
				$DmiSentEmailLogsEntity = $DmiSentEmailLogs->newEntity(array(
					...
					'message_id'=>$message_id,
					'destination_list'=>base64_encode($destination_email_ids_values),
					'sent_date'=>date('Y-m-d H:i:s'),
					'message'=>$sms_message,
					'created'=>date('Y-m-d H:i:s'),
					'template_id'=>$template_id //added on 12-05-2021 by Amol
	...
	public function getReplaceDynamicValues($replace_variable_value,$userCode,$sample_code) {
		...
		switch ($replace_variable_value) {
			...
			case "src_user":
				return $src_user_name['f_name']." ".$src_user_name['l_name'];
			break;
			...
			case "dst_user":
				return $dst_user_name['f_name']." ".$dst_user_name['l_name'];
			break;
			...
			case "sample_date":
				return $sampleDetails['created'];
			break;

			case "letr_ref_no":
				return $sampleDetails['letr_ref_no'];
			break;

			case "ref_src_code":
				return $sampleDetails['ref_src_code'];
			break;

			case "exp_sample":
				return $month_name." ".$sampleDetails['expiry_year'];
			break;

			case "oic":
				return $getoicdetails['f_name']." ".$getoicdetails['l_name'];
			break;

			case "pao_name":
				return $getPao['f_name']." ".$getPao['l_name'];
			break;

			case "ro_name":
				return $getRo['f_name']." ".$getRo['l_name'];
			break;

			case "chemist_name":
				return $getChemist['f_name']." ".$getChemist['l_name'];
			break;

			case "inward":
				return "Inward"." ".$getInward['f_name']." ".$getInward['l_name'];
			break;
