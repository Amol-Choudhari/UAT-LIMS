...
/****************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleToApproveReading() {
        ...
        $query = $conn->execute("SELECT c.sample_code
								 ...
								 AND w.dst_usr_cd='".$_SESSION['user_code']."' AND w.stage_smpl_flag='FT' AND si.status_flag NOT IN ('SR','junked') GROUP BY c.sample_code");
...
/*******************************************************************************************************************************************************************************************************************************************/
	...
	public function forwardRal(){
		...
		$this->DmiSmsEmailTemplates->sendMessage(105,$oic,$sample_code,); 					#OIC
...
/*******************************************************************************************************************************************************************************************************************************************/
	...
	public function approvedResults(){
		...
		#DISTINCT clause is added to the below code to avoid the duplicate codes in the list of approved list - Akash[10-02-2023]
		#ORDER BY clause is added to sort the results by transaction date - Akash[10-02-2023]
		#=> Query Updated :: /* New status Flag is added i,e status_flah != 'junked' - Shankhpal*/
		$query = $conn->execute("SELECT DISTINCT w.stage_smpl_cd,mcc.category_name,mc.commodity_name,mst.sample_type_desc,w.tran_date 
								 FROM workflow AS w
