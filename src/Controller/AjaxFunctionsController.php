...
/*******************************************************************************************************************************************************************************************************************************/
	...
	public function searchSample(){
		...
		//to check sample code is junked or not by shankhpal shende on 10/03/2023
		$conn = ConnectionManager::get('default');
		
		// select sample code where status is junked
		...
		if(!empty($junk_sample)){
			// if SampleType,commodity,location is null then set default value NA by shankhpal shende
			if(empty($SampleType) OR empty($commodity) OR empty($location)) {
				$SampleType .= 'NA';
				$commodity .= 'NA';
				$location .= 'NA';
			}
