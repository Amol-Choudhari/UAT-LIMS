...
/************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleToAccept() {
		...
		$testalloc1 = $conn->execute("SELECT DISTINCT b.chemist_code,a.test_name FROM m_sample_allocate AS s
									...
									WHERE w.stage_smpl_cd NOT IN ('','blank') AND alloc_cncl_flag='N' AND acptnce_flag='N' AND si.display='Y' AND s.display='Y' AND s.alloc_to_user_code=$alloc_user
									ORDER BY a.test_name ASC");

									// added inner join with workflow and added condition for stage_smpl_cd != ''
									// si.status_flag !='junked' for junk sample
...
/************************************************************************************************************************************************************************************************************************/
	...
	public function acceptSampleBychemist() {
		...
		for ($i=0;$i<count($data);$i++) {
			...
			$workflow_data[] = array("org_sample_code"=>$sample[0]['org_sample_code'], 
									 "src_loc_id"=>$sample[0]['lab_code'], 
									 "src_usr_cd"=>$sample[0]['alloc_by_user_code'],
									 "dst_loc_id"=>$sample[0]['lab_code'],
									 "dst_usr_cd"=>$sample[0]['alloc_to_user_code'],
									 "stage_smpl_cd"=>$sample[0]['sample_code'],
									 "user_code"=>$_SESSION["user_code"],
									 "tran_date"=>$tran_date, 
									 "stage"=>$stage,
									 "stage_smpl_flag"=>"TABC");
		...
		foreach($workflowEntity as $each){
			...
			$chemistCode = $this->Workflow->find()->select(['stage_smpl_cd'])->where(['org_sample_code' => $each['org_sample_code'], 'stage_smpl_flag' => 'TA'])->first();
	
			#SMS : Sample Accept for Test
			$this->DmiSmsEmailTemplates->sendMessage(96,$each['dst_usr_cd'],$chemistCode['stage_smpl_cd']); #Chemist
			$this->DmiSmsEmailTemplates->sendMessage(97,$each['src_usr_cd'],$each['stage_smpl_cd']); #Inward
...
/************************************************************************************************************************************************************************************************************************/
	...
	public function allocCancel() {
		...
		for($i=0;$i<count($data);$i++){
			...
			$conn->execute("UPDATE m_sample_allocate SET alloc_cncl_flag='C',acptnce_flag='NABC',sendback_remark='$sendback_remark',recby_ch_date='$date' WHERE sr_no=".$data[$i]);
			$query = $conn->execute("SELECT chemist_code FROM m_sample_allocate WHERE sr_no=".$data[$i]);
			$qr_res = $query->fetchAll('assoc');
			$chemist_code[]=$qr_res[0]['chemist_code'];
			...
			#This Below query is commented to see if there is any impact , as the delete query should not be used in programming - Akash [02-03-2023]
		...
		foreach ($workflowEntity as $each) {
			...
			$oic = $this->DmiRoOffices->getOfficeIncharge($each['dst_loc_id']);
			$chemistCode = $this->Workflow->find()->where(['org_sample_code' => $each['org_sample_code'], 'stage_smpl_flag' => 'TA'])->first();
		
			#SMS: Sample Rejected For Test
			$this->DmiSmsEmailTemplates->sendMessage(98,$chemistCode['dst_usr_cd'],$chemistCode['stage_smpl_cd']); #Chemist
			$this->DmiSmsEmailTemplates->sendMessage(99,$each['dst_usr_cd'],$each['stage_smpl_cd']); #Inward
...
/************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleToEnterReading() {
		...
		// query updated: for added new condition sa.chemist_code !=''    
		...
		$query = $conn->execute("SELECT DISTINCT ON (sa.chemist_code) sa.chemist_code,
									...
									WHERE si.status_flag !='junked' AND w.stage_smpl_cd NOT IN ('','blank') AND sa.chemist_code !='' AND sa.chemist_code ".$arr."");
									// stage_smpl_cd !='' condition added for empty sample code
									// si.status_flag !='junked'
									// by shankhpal shende on 19/04/2023
...
/************************************************************************************************************************************************************************************************************************/
	...
	public function getIncompleteTest() {
		...
		if ($flag) {
			...
			echo "#NULL#"; #this else block is added to avoid the errors in the chemist readings - Akash[18-02-2023]
		}
	...
	public function viewPrfmTest() {
		...
		if (isset($_SESSION['username'])) {
			...
			$query = $conn->execute("SELECT DISTINCT(w.stage_smpl_cd),
									...
									WHERE si.status_flag !='junked'  AND w.stage_smpl_cd NOT IN ('','blank') AND w.src_usr_cd='$alloc_user' AND w.stage_smpl_flag='FT'");
									//  status_flag !='junked' condition use for junk sample 
									// added by shankhpal shende on 20/04/2023 
...
/************************************************************************************************************************************************************************************************************************/
		...
		foreach ($test_string as $row) {
			...
			if (isset($res2[0]['count'])>0) {
				...
				if (count($test_result)>0) {
					...
					$result="Not Finalized"; // before finalized sample chemist window By Shreeya
				}
			}
	...
	// This function is used for to genrate calculate chemist test report,
	//20-01-2023 By Shreeya
	public function calculateChemTestReportCode($sample_code,$commodity_code) {

		$this->Session->write('ABchmist_sample_code12',$sample_code);
		$this->Session->write('ABcommodity_code12',$commodity_code);
		$this->redirect(array('controller'=>'Test','action'=>' calculate_chem_test_report'));

	}


	// This function is used for to genrate chemist calculate test report on 20-01-2023 By Shreeya
	public function calculateChemTestReport() {
