...
/************************************************************************************************************************************************************************************************************************/
			...
			if (null !== ($this->request->getData('ok'))) {
				...
				if (!empty($this->request->getData('f_name')) && !empty($this->request->getData('l_name')) && !empty($this->request->getData('phone')) /*&& !empty($this->request->getData('once_card_no'))*/) {
					...
					if (preg_match("/^[X-X]{6}[0-9]{4}$/i", $this->request->getData('phone'),$matches)==1){
						...
						$htmlencodedphone = base64_encode($htmlencodedphone); // This added to encode the value - Akash[12-01-2023]
					}
					...
					if ($this->DmiUsers->save($DmiusersEntity)) {
						...
						$DmiUserHistoryLogsEntity = $this->DmiUserHistoryLogs->newEntity(array(
							...
							'profile_pic'=>$profile_pic, //added on 06-05-2021 for profile pic
							'done_by'=>$_SESSION['username'] // added by shankhpal shende on 12/01/2023
	...
	public function userPendingWorkTransfer() {
		...
		$authorized_user = $this->DmiUsers->find('all')->select(['id'])->join(['DmiUserRoles'=> ['table' => 'dmi_user_roles', 'type' => 'INNER','conditions' => ['DmiUserRoles.user_email_id = DmiUsers.email', 'DmiUserRoles.super_admin' => 'yes','DmiUsers.id IS' => $this->Session->read('user_code')]]])->first();
				...
				if (!empty($from_user) && !empty($to_user) && !empty($reason) ) {
					...
					if (is_numeric($from_user) && is_numeric($to_user)) {
						...
						if (count($valid_user) == 2 ) {
							...
							//17-01-2023 shreeya, !empty condition added
							if (!empty($chemistcode)) {
							...
							if (!empty($finalPendingList)) {
								//add a blank array for org_smpl_code 
								$original_code= array();
								...
								foreach ($theWorkflowModelEntity as $each) {	
									...
									//add a not empty condition for update a sample inward query
									if(!empty($original_code)){
										$this->SampleInward->updateAll(array('user_code'=>$to_user),array('org_sample_code IN'=>$original_code));
									}

									//Chemist work transfer entry is not show to code decode table when work is transferred  on 18-01-2023 by shreeya
									if(!empty($original_code)){
										//check the role of chemist
										$fromuserrole = $this->DmiUsers->find('all',array('fields'=>'role','conditions'=>array('id IS'=>$to_user),'role IN' => array('Jr Chemist','Sr Chemist')))->toArray();
										if(!empty($fromuserrole)){
											//Added an update query when the work Transfer  codeDecode table is not updating alloc_to_user_code code on 20-01-2023
											$this->CodeDecode->updateAll(array('alloc_to_user_code'=>$to_user),array('alloc_to_user_code'=>$from_user,'org_sample_code IN'=>$original_code));
										}
									}
								}
							}
