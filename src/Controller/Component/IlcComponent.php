<?php
	...
	class IlcComponent extends Component {
		...
		// added for selected  record save or not check for testname done 09/11/2022 by shreeya
		public function savedornot($forw_sample_cd) {

			$IlcSaveTestParameters = TableRegistry::getTableLocator()->get('IlcSaveTestParameters');
			$SavedList = $IlcSaveTestParameters->find('all')->select()->where(['status IS' => '1','sample_code'=>$forw_sample_cd])->toArray();
		...
		public function getSavedSelectedRALs($sampleTypeCode) {
			...
			$query = $conn->execute("SELECT u.id as urid,ro.id as rid ,u.f_name,u.l_name,ro.ro_office,sr.qty FROM ilc_selected_rals AS sr
		...
		//added for save seleted test name done by shreeya on 09-11-2022
		public function SavedSelectedTestname($sampleTypeCode) {
			...
			// featch saved list on template done 14/06/2022 by shreeya
			$getSavedList = $IlcSaveTestParameters->find('all')->select()->where(['status IS' => '1','sample_code'=>$forw_sample_cd,'sample_type'=>$sampleTypeCode])->toArray();
		
			$sql = $conn->execute("SELECT m.test_code  as tid ,m.test_name FROM ilc_save_test_parameters AS stp
									INNER JOIN m_test AS m ON m.test_code = stp.testname
									WHERE stp.sample_code='$forw_sample_cd'AND stp.sample_type='$sampleTypeCode'AND stp.status=1 order by stp.id");

			$SavedList = $sql->fetchAll('assoc');
			
			$this->Controller->set('SavedList',$SavedList);
		}
			...
			if (!empty($final_result_details1)) {
				...
				foreach ($final_result_details1 as $stage_sample_code) {
					...
					if (!empty($final_grading_details1)) {
						...
						$final_grading1 = $Workflow->find('all',array('conditions'=>array('dst_usr_cd'=>$user_id,'id'=>$final_grading_details1['id'],'stage_smpl_flag !='=>'FG')))->first();
		...
		//added for calculate standard deviation 
