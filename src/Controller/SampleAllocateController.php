			...
			if ($this->request->is('post')) {
				...
				if (isset($postData['save'])) {
					...
					if ($allocateResult) {
						...
						//below code added for when chemist code is not generate [ Date - 13-04-2023 by shreeya ]
						if (empty($stage_smpl_cd)) {

							$this->set('validate_err'," Sample Not Allocate Please Try Again ");
							return null;
						}
...
/********************************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleToAllocate($from_dt=null,$to_dt=null){
		...
		#=> Query Updated :: new condition of stage sample code to distinct records [Akash]
		#=> Query Updated :: /* New status Flag is added i,e status_flah != 'junked' - Shankhpal*/
		$query = $conn->execute("SELECT DISTINCT ON (w.stage_smpl_cd) si.inward_id,w.stage_smpl_cd,
								si.received_date,st.sample_type_desc,mcc.category_name,
								mc.commodity_name,ml.ro_office,w.modified AS accepted_on
								FROM sample_inward AS si
								...
								INNER JOIN workflow AS w ON w.org_sample_code = si.org_sample_code
								WHERE si.status_flag !='junked' AND w.stage_smpl_cd NOT IN ('','blank') AND w.stage_smpl_cd IN ('$cus_string')".$dateCondition);
								// added status_flag != junked by shankhpal shende
...
/********************************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleToForwardInchrg($from_dt=null,$to_dt=null){
		...
		//to get extra parameters for listing
		$query = $conn->execute("SELECT DISTINCT ON (w.stage_smpl_cd) si.inward_id,
								w.stage_smpl_cd,si.received_date,st.sample_type_desc,
								mcc.category_name,mc.commodity_name,ml.ro_office,w.modified AS accepted_on
								FROM sample_inward AS si
								INNER JOIN m_sample_type AS st ON si.sample_type_code=st.sample_type_code
								INNER JOIN m_commodity_category AS mcc ON si.category_code=mcc.category_code
								INNER JOIN dmi_ro_offices AS ml ON ml.id=si.loc_id
...
/********************************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleReturnedByChemist($from_dt=null,$to_dt=null){
		...
		$sendback = $conn->execute("SELECT  DISTINCT ON (s.sample_code) s.sample_code,b.commodity_name,s.chemist_code,s.sendback_remark,s.recby_ch_date,s.org_sample_code
									...
									AND a.status_flag!='SR' AND wf.stage_smpl_cd !=''
...
/********************************************************************************************************************************************************************************************************************************************/
	...
	public function allocatedList(){
		...
		if (!empty($from_dt) || !empty($to_dt)) {
			...
			#=> Query Updated :: /* New status Flag is added i,e status_flah != 'junked' - Shankhpal*/
			$allRes = $conn->execute("SELECT a.sample_code,cd.chemist_code,
											 alloc_date,u.f_name || ' ' || u.l_name AS f_name,
											 a.alloc_to_user_code,cun.f_name || ' ' || cun.l_name AS cun_f_name
									  FROM actual_test_data AS a
									  INNER JOIN code_decode AS cd ON cd.chemist_code=a.chemist_code 
										AND a.alloc_to_user_code=cd.alloc_to_user_code 
										AND cd.display='Y' AND a.sample_code=cd.sample_code
									  INNER JOIN m_sample_allocate AS sa ON sa.chemist_code=cd.chemist_code 
										AND sa.alloc_to_user_code=cd.alloc_to_user_code 
										AND sa.display='Y' AND sa.sample_code=cd.sample_code AND sa.test_n_r='N'
									  INNER JOIN dmi_users AS cun ON cun.id=sa.alloc_to_user_code
									  INNER JOIN sample_inward AS si ON si.org_sample_code = a.org_sample_code 
									  INNER JOIN dmi_users AS u ON u.id=sa.user_code 
									  INNER JOIN workflow AS w ON w.org_sample_code = si.org_sample_code
									  WHERE si.status_flag !='junked' AND w.stage_smpl_cd NOT IN ('','blank')
										AND u.role='".$_SESSION['role']."' 
										AND a.display='Y' 
										AND cd.chemist_code!='-' 
										AND cd.user_code='".$_SESSION['user_code']."' 
										AND date(a.created) >= '$from_dt' 
										AND date(a.created) <= '$to_dt' 
									  GROUP BY a.sample_code,u.f_name,cun.f_name,cd.chemist_code,
									  		   sa.test_n_r,sa.alloc_date,u.l_name,cun.l_name,
											   a.alloc_to_user_code 
									  ORDER BY sa.alloc_date DESC");
									// added by shankhpal shende stage_smpl_cd != '' on 19/04/2023
									//added line for Inner join workflow As w - on 12-05-2023 as w used but not declared, by Amol
			...
			foreach ($allRes as $allRes2) {
				...
				$allRes1 = $conn->execute("SELECT a.chemist_code,b.test_name
										   ...
										   INNER JOIN m_sample_allocate AS sa ON sa.chemist_code=cd.chemist_code 
										   AND sa.alloc_to_user_code=cd.alloc_to_user_code 
										   AND sa.display='Y' AND sa.sample_code=cd.sample_code
										   ...
										   AND a.display='Y' 
										   AND cd.chemist_code!='-' 
										   AND a.alloc_to_user_code='$user_code_new'
										   AND a.chemist_code='$chemist_code'
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>[Get Sample To Forward Incharge Retest]>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	public function getSampleToForwardInchrgRetest($from_dt=null,$to_dt=null){
		...
		$query = $conn->execute("SELECT DISTINCT ON (w.stage_smpl_cd) si.inward_id, w.stage_smpl_cd, si.received_date, st.sample_type_desc, mcc.category_name, mc.commodity_name, ml.ro_office, w.modified AS requested_on
								 ...
								 INNER JOIN workflow AS w ON w.org_sample_code = si.org_sample_code
								 WHERE si.status_flag !='junked' AND w.stage_smpl_cd NOT IN ('','blank') AND w.stage_smpl_cd IN ('$cus_string')".$dateCondition);
   								// status_flag, stage_smpl_cd !='' added by shankhpal shende 
...
/********************************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleReturnedByChemistRetest($from_dt=null,$to_dt=null){
		...
		$sendback = $conn->execute("SELECT DISTINCT s.sample_code, b.commodity_name, s.chemist_code, s.sendback_remark, s.recby_ch_date, s.org_sample_code
									...
									INNER JOIN workflow AS wf ON wf.org_sample_code=a.org_sample_code
									INNER JOIN m_commodity AS b ON a.commodity_code=b.commodity_code
									WHERE a.status_flag !='junked' AND wf.stage_smpl_cd != '' AND s.acptnce_flag='NABC' AND s.test_n_r='R' AND s.user_code='".$_SESSION['user_code']."'".$dateCondition);
									//status_flag, stage_smpl_cd != '' added by shankhpal shende 
...
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>[allocatedRetestList]>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
	...
	public function allocatedRetestList(){
		...
		$allRes = $conn->execute("SELECT a.sample_code,cd.chemist_code, 
										alloc_date,u.f_name || ' ' || u.l_name AS f_name,
										a.alloc_to_user_code,
										cun.f_name || ' ' || cun.l_name AS cun_f_name
								FROM actual_test_data AS a
								INNER JOIN code_decode AS cd ON cd.chemist_code=a.chemist_code 
								...
								INNER JOIN dmi_users AS cun ON cun.id=sa.alloc_to_user_code
								INNER JOIN dmi_users AS u ON u.id=sa.user_code  
								INNER JOIN sample_inward AS si ON a.org_sample_code = si.org_sample_code
								WHERE si.status_flag !='junked' AND w.stage_smpl_cd NOT IN ('','blank')
									AND u.role='".$_SESSION['role']."'
									AND a.display='Y' 
									AND cd.chemist_code!='-'  
									AND cd.user_code='".$_SESSION['user_code']."'
								GROUP BY a.sample_code,u.f_name,cun.f_name,
										 cd.chemist_code,sa.test_n_r,sa.alloc_date,
										 u.l_name,cun.l_name,a.alloc_to_user_code 
								ORDER BY alloc_date DESC");
					// inner join with sample_inward and added condition status_flag != junked by shankhpal shende
					// w.stage_smpl_cd != '' added by shankhpal shende as per change
		...
		foreach ($allRes as $allRes2) {
			...
			$allRes1 = $conn->execute("SELECT a.chemist_code,b.test_name
									   ...
									   GROUP BY a.chemist_code,b.test_name");
									// inner join with sample_inward and added condition status_flag != junked by shankhpal shende
									// stage_smpl_cd != '' added by shankhpal shende as per change req.
