...
class DashboardController extends AppController{
	...
	public function dashboardpendingWorkCount($check_user_role){
		...
		if (!empty($check_user_role)) {
			...
			if ($check_user_role['finalized_sample'] == 'yes') {
				...
				if ($_SESSION['role']=="Inward Officer") { 
					//to grade by Inward pending status could not show propely add count of [0],[1] done by shreeya on 12-01-2023
					$main_tab_count['to_grade_by_inward'] = count(((new FinalGradingController())->getSampleToGradeByInward())[0])+count(((new FinalGradingController())->getSampleToGradeByInward())[1]);

				} else {  
					//to grade by OIC pending status could not show propely add count of [0],[1] done by shreeya on 12-01-2023
					$main_tab_count['to_grade_by_oic'] = count(((new FinalGradingController())->getSampleToGradeByOic())[0])+count(((new FinalGradingController())->getSampleToGradeByOic())[1]);
	...
	public function getFlagWiseActivity($stageSampleFlag){
		...
		}elseif($stageSampleFlag == 'PS' || $stageSampleFlag == 'PV' || $stageSampleFlag == 'PR' || $stageSampleFlag == 'PC'){ // For commercial 
