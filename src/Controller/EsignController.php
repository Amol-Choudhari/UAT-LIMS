...
class EsignController extends AppController {
	...
	public function createEsignXmlAjax(){
		...
		$doc_path = $_SERVER["DOCUMENT_ROOT"].'/testdocs/LIMS/reports/'.$pdf_file_name;		
		...
		$response_url = 'https://agmarkonline.dmi.gov.in/UAT-LIMS/esign/request_esign';
...
//This function is created to append response signature on existing pdf doc.
	public function signTheDoc($resp_array,$pdf_file_name){
		...
		if($resp_status == 1){
			...
			$pdf_path = $_SERVER["DOCUMENT_ROOT"].'/testdocs/LIMS/reports/'.$pdf_file_name;
