...
class ReportController extends AppController
{
	...
	public function testReportForCommodity()
	{
		if ($this->request->is('post')) {
			$sample_code = $this->request->getData('sample_code');
			//  Updated below redirection conditionally if file found open it, or generate the old report 
			//  dATE : 15-02-2023 Done By Shreeya
			if(!empty($sample_code)){
				$this->loadModel('SampleInward');
				$getSampleType = $this->SampleInward->find('all', array('fields' => 'report_pdf', 'conditions' => array('org_sample_code IS' => $sample_code)))->first();
				 
				if(!empty($getSampleType)){
					$reportpdf = $getSampleType['report_pdf'];
					if(!empty($reportpdf)){
						$this->redirect('/../'.$reportpdf);
						...
						if (!empty($records)) {
							...
							if ($test_report) {
								...
								//below code commented on 17-02-2023 by Amol					
	...
	public function finalSampleTestReports()
	{
		...
		if (!empty($records)) {
			foreach ($records as $sample_code) {
				...
				$final_grading = $con->execute("SELECT w.stage_smpl_cd, w.tran_date, mcc.category_name, mc.commodity_name, mst.sample_type_desc, mc.commodity_code,si.report_pdf
				FROM workflow w
				...
				INNER JOIN m_sample_type mst ON mst.sample_type_code = si.sample_type_code
				WHERE si.status_flag != 'junked' AND w.stage_smpl_flag = 'FG' AND w.org_sample_code = '$sample_code'");
