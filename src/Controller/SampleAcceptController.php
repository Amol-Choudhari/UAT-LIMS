	...
	public function sampleAccept(){
		...
		if(!empty($accpt_sample_cd)){
			...
			if($sampleTypeCode == 9){
				...
				$query = $conn->execute("SELECT sm.inwd_off_val,sr.qty 
											FROM ilc_org_smplcd_maps AS sm 
											INNER JOIN ilc_selected_rals AS sr ON sm.inwd_off_val = sr.inwd_off_val
											WHERE sr.stage_sample_code = '$orgsamplecode' AND sm.ilc_org_sample_cd = '$sample_code' 
											ORDER BY sr.id DESC");
		...
		if ($_SESSION['user_flag']=='HO') {
			...
			$query = $conn->execute("SELECT TRIM(w.stage_smpl_cd) AS stage_sample_code
									 ...
									 INNER JOIN  workflow AS w ON w.org_sample_code=si.org_sample_code 
										AND si.display='Y'
										AND si.status_flag='H'  
										AND w.stage_smpl_flag='HF' 
										AND w.stage_smpl_cd NOT IN ('','blank') 
										AND w.dst_usr_cd='".$_SESSION['user_code']."'
									 ORDER BY w.id DESC");
									 // w.stage_smpl_cd NOT IN ('','blank') added by shankhpal shende for empty sample code
		} else {
			...
			$query = $conn->execute("SELECT TRIM(w.stage_smpl_cd) AS stage_sample_code
									 ...
									 INNER JOIN  workflow AS w On w.org_sample_code=si.org_sample_code 
										AND si.display='Y' 
										AND si.status_flag='F'  
										AND w.stage_smpl_cd NOT IN ('','blank')  
										AND w.stage_smpl_flag='OF' 
										AND w.dst_usr_cd='".$_SESSION['user_code']."'
									 ORDER BY w.id DESC");
									// w.stage_smpl_cd NOT IN ('','blank') added by shankhpal shende for empty sample code
		}
...
/********************************************************************************************************************************************************************************************************************************/
	...
	public function getCommodityObs(){
		...
		$query="SELECT mso.m_sample_obs_code,mso.m_sample_obs_desc 
				...
				INNER JOIN m_commodity_obs AS mco ON mco.m_sample_obs_code=mso.m_sample_obs_code
					AND mco.category_code='$category_code' 
					AND mco.commodity_code='$commodity_code'
...
/********************************************************************************************************************************************************************************************************************************/
	...
	public function getCommodityObs1(){
		...
		$query	= "SELECT mco.m_sample_obs_type_code,mco.m_sample_obs_type_value,mco.m_sample_obs_code 
					FROM m_sample_obs AS mso
					INNER JOIN m_sample_obs_type AS mco ON mco.m_sample_obs_code=mso.m_sample_obs_code 
						AND mso.m_sample_obs_code ='$sample_obs_code'
...
/********************************************************************************************************************************************************************************************************************************/
	...
	public function getCommodityRgs(){
		...
		if (isset($_POST['sample_code'])) {
			...
			$query1="SELECT stage_sample_code 
						FROM sample_inward 
						WHERE inward_id='$sample_code'";
			...
			$query2="SELECT m_sample_reg_obs_code,m_sample_obs_code,m_sample_obs_type_code 
						FROM m_sample_reg_obs 
						WHERE stage_sample_code='$sample_code1'";
	...
	public function getUser(){
		...
		$this->loadModel('DmiUsers');
		  ...
		  if ($flag=="HO") {
				...
				$query = $conn->execute("SELECT u.id,u.f_name,u.l_name 
											FROM dmi_users AS u
											INNER JOIN dmi_user_roles AS ur ON u.email=ur.user_email_id
											WHERE posted_ro_office=$dst_loc_id AND ur.user_flag='$flag' 
											AND u.id=".$_SESSION["user_code"]." ");
		  } else {

				$query = $conn->execute("SELECT u.id,u.f_name,u.l_name 
											FROM dmi_users AS u
											INNER JOIN dmi_user_roles AS ur ON u.email=ur.user_email_id
											WHERE posted_ro_office=$dst_loc_id 
											AND role='Inward Officer'
											AND ur.user_flag='$flag'
											AND u.id=".$_SESSION["user_code"]." ");
			}
			...
			if($_SESSION['user_flag']=='RO' || $_SESSION['user_flag']=='SO'){
				...
				$query1=$conn->execute("SELECT ur.user_flag,o.id,o.ro_office 
										...
										INNER JOIN user_role AS r ON r.role_name=u.role 
											AND ur.user_flag='RAL' 
											AND u.posted_ro_office=$location
	...
	public function frdLetterPdf($stage_sample_code){
		...
		$query = $conn->execute("SELECT si.*,b.sample_type_desc,c.container_desc,a.unit_weight,w.dst_usr_cd,w.dst_loc_id,w.src_usr_cd,w.src_loc_id,w.stage_smpl_cd,m.commodity_name
								 ...
								 WHERE w.stage 
									...
									AND si.status_flag
										IN('F','H')
										AND w.stage_smpl_cd='$sample_code'");
...
/********************************************************************************************************************************************************************************************************************************/
	...
	public function getSampleListToAccept(){
		...
		$query = $conn->execute("SELECT  si.inward_id, w.stage_smpl_cd, si.received_date, st.sample_type_desc, mcc.category_name, mc.commodity_name, ml.ro_office, w.modified AS forwarded_on
									...
									INNER JOIN workflow AS w ON w.org_sample_code = si.org_sample_code
									WHERE si.status_flag !='junked' AND w.stage_smpl_cd IN ('$cus_string') AND w.stage_smpl_cd NOT IN ('','blank') 
									ORDER BY w.modified DESC");
									// stage_smpl_cd !='' added by shankhpal shende as per change req. on 19/04/2023
