				...
				$.ajax({
						...
						success: function (data) {
							...
							if(resArray.indexOf('[error]') !== -1){
								...
								// The below code was commented by Shreeya when the accepted reading window was not displayed as per the test formula of the average button
								// Date : 03-04-2023]
								/*var lp=0;
								...
								var ts= 0;
								// Commented out this line, adding a new loop By shreeya on [Date : 03-04-2023]
								...
								$.each(resArray, function (key, value) {
									...
									//Commented out this line, adding a new loop to the existing result to show value By shreeya on [Date : 03-04-2023] 
									//$.each( value,function (key1, value1) { 
									$.each( arr1,function (key1, value1) {
									...
									//Added by AkashT to match the tests and seperate the test code , test name and results and show in thee table.-->
										var str=key1+'~'+value1+'~'+para3; /* add para3 for show test accorinding to formula By shreeya on [Date : 03-04-2023] */
  ...
  //Below code added by AkashT to set the Color to the ROW based on Approved [GREEN] & Unapproved [ORANGE] test results.-->
					...
					success: function (data) {
						...
						if(test_type=='Formula'||  test_type=='RT'|| test_type=='SV'|| duplicate_flag =='D')
						{
							for(i=0;i<resultarr.length;i++)
							{
								...
								//added this condition for check Number is decimal or Not
								var value = resultarr[0];
								var regex = /^[-+]?[0-9]+\.[0-9]+$/;
								var isValidated = regex.test(value);
								if (isValidated) {
									//count the length of chemist value and split value after decimal
									len_of_deci_res = resultarr[0].toString().split('.')[1].length;
								}
								else {
									len_of_deci_res = 0;
								}
							...
							//split average value after decimal according to chemist final result.
							//Done By - Shreeya [Date: 05-05-2023]
							var totalArr = avg1.toString().split('.');
							var totalTruncated = (totalArr.length == 2) ? totalArr[0] + '.' + totalArr[1].toString().substr(0, len_of_deci_res) : avg1;

							// avg=avg1.toFixed(4);
							// defined the variable for average value
							avg=totalTruncated;
							if(fr==avg){
	...
	function get_final_result(result,test_code1,test_name,test_type,r_id1,chem_code,duplicate_flag,sam_code)
	{
	 ...
	 //Added by AkashT for Array-->
		...
		if (isValidated) {
			//count the length of chemist value and split value after decimal
			len_of_res = resultarr[0].toString().split('.')[1].length;
		}
		else {
			len_of_res = 0;
		}
		...
		//Done By - Shreeya [Date: 05-05-2023]
		var totalArr = avg1.toString().split('.');
		var totalTruncated = (totalArr.length == 2) ? totalArr[0] + '.' + totalArr[1].toString().substr(0, len_of_res) : avg1;
