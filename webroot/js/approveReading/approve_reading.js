$(document).ready(function(){
		...
		if (sample_code != "") {
			...
			$.ajax({
				...
				success: function (data) {
					...
					$.each(resArray, function (key, value){
						alert("The finalized result has been sent to ho,"+value['ro_office']+" for verification !!!");
			...
			$.ajax({
				...
				success: function (data) {
					...
					$.each(resArray, function (key, value){
						...
						// TRIM function is added to the below varible to trim the whitespaces - Akash[10-02-2023]
						if($.trim(sess_user_flag) == 'RAL'){
						...
						// TRIM function is added to the below varible to trim the whitespaces - Akash[10-02-2023]
						}else if($.trim(sess_user_flag) == 'CAL'){
			...
			success: function (data) {
				...
				if(resArray.indexOf('[error]') !== -1){
					...
					if(resArray.indexOf('Exists') !== -1){
						...
						// TRIM function is added to the below varible to trim the whitespaces - Akash[10-02-2023]
						if($.trim(sess_user_flag) == 'RAL' && $.trim(sess_role) == 'Inward Officer'){
						...
						// TRIM function is added to the below varible to trim the whitespaces - Akash[10-02-2023]
						} else if($.trim(sess_user_flag) == 'RAL' && $.trim(sess_role) == 'Lab Incharge') {
						...
						// TRIM function is added to the below varible to trim the whitespaces - Akash[10-02-2023]
						}else if($.trim(sess_user_flag) == 'CAL' && $.trim(sess_role) == 'Lab Incharge'){
						...
						// TRIM function is added to the below varible to trim the whitespaces - Akash[10-02-2023]
						}else if($.trim(sess_user_flag) == 'CAL' && $.trim(sess_role) == 'Inward Officer'){
								...
								success: function (data) {
									...
									if(resArray.indexOf('[error]') !== -1){
										...
										$.each(resArray, function (key, value){
											...
											$.alert("All test for Sample "+value['sample_code']+" has been Finalized  !!!");
	...
	$("#save").click(function (e) {
		...
		$.ajax({
			...
			url: 'approve_reading',
			data: {
				duplicate_flg:duplicate_record,
				...
				if(resArray.indexOf('[error]') !== -1){
					...
					$.each(resArray, function (key, value){
						...
						$.alert({
							...
							content: "Test result for <b>"+value['test_name']+"</b> is approved!!",
							onClose: function(){
								view1();
							}
						});
