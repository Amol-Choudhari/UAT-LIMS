									...
									<tbody>
										<?php if (isset($final_sample_reports)) {
												...
												<tr>
													...
													<!-- Updated below links conditionally if file found open it, or generate the old report on 11-01-2023 by Amol-->
													<?php if (!empty($res2['report_pdf'])) { ?>
														<td><a href="<?php echo $res2['report_pdf']; ?>" target='_blank' class="btn btn-info">View</a></td>
													<?php } else { ?>
														<td><a href="<?php echo $this->request->getAttribute('webroot'); ?>FinalGrading/sample_test_report_code/<?php echo trim($res2['stage_smpl_cd']) . '/' . $res2['commodity_code']; ?>" target='_blank' class="btn btn-info">View</a></td>
													<?php } ?>
												</tr>
										<?php $i=$i+1; } } ?>
									</tbody>
